...
build_modules() {
    ...
    if [ -d "$RACO_DIR" ]; then
        ...
        RACO_ZIP_NAME="${MODULE_ID}-RacoPlugin-${VERSION}-${BUILD_TYPE}.zip"
        RACO_ZIP_PATH="../$BUILD_DIR/$RACO_ZIP_NAME"
        zip -q -r "$RACO_ZIP_PATH" ./*
        echo "Created: $RACO_ZIP_NAME"
        ...
        if prompt_telegram_post; then
            ...
            if select_telegram_groups; then
                ...
                for i in "${!SELECTED_GROUPS[@]}"; do
                    ...
                    # Upload the RacoPlugin zip file if it exists
                    if [ -n "$RACO_ZIP_NAME" ]; then
                         if [ -f "$BUILD_DIR/$RACO_ZIP_NAME" ]; then
                             caption="📱 $MODULE_ID Raco Plugin - $VERSION ($BUILD_TYPE)"
                             if ! send_to_telegram "$BUILD_DIR/$RACO_ZIP_NAME" "$caption" "$chat_id"; then
                                 group_upload_ok=false
                             fi
                         fi
                    fi

                    if [ "$group_upload_ok" = true ]; then
                        ((upload_success++))
                        ...
                        FAILURE_MESSAGE="❌ *Upload Failed*%0A%0AThere was an issue uploading to $group_name."
